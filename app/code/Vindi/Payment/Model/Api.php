...
class Api extends \Magento\Framework\Model\AbstractModel
{
    ...
    public function __construct(
        ...
        \Vendor\Cachetype\Model\Cache\Type $cacheType,
    ...
    )
    {
        ...
        $isSandBox = $this->scopeConfig->getValue('payment/vindi/sandboxmode', $storeScope);

        if ($isSandBox) {
            $this->base_path = 'https://sandbox-app.vindi.com.br/api/v1/';
        } else {
            $this->base_path = 'https://app.vindi.com.br/api/v1/';
        }

        $moduleInfo = $moduleList->getOne('Vindi_Payment');
        $this->version = $moduleInfo['setup_version'];
        $this->key = $this->scopeConfig->getValue('payment/vindi/key', $storeScope);
    }
    ...
    {
        $storeScope = \Magento\Store\Model\ScopeInterface::SCOPE_STORE;
        $notifyEmail = $this->scopeConfig->getValue(self::BASEPATH, $storeScope);
    ...
    public function buildPlanItemsForSubscription($order)
    {
        ...
        foreach ($orderItems as $item) {
            ...
            if ($product->getTypeID() !== 'subscription') {
                throw new \Exception("O produto {$item->getName()} não é uma assinatura.");
                return false;
            }
